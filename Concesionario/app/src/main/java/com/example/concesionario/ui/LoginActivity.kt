...
class LoginActivity : AppCompatActivity() , OnClickListener,RadioGroup.OnCheckedChangeListener{
    ...
    private fun instancias(){
        usuarios= arrayListOf(Usuario(nombre = "Borja","borja@gmail.com","1234"),
            Usuario(nombre = "Juan","juan@gmail.com","4567"),
            Usuario(nombre = "Maria","maria@gmail.com","7891"))
    }
    private fun initGUI(){
    ...
    override fun onClick(v: View?) {
        when(v?.id){
            binding.btnAcceso.id->{
                //cambiamos de escena -> si es invitado no preguntamos por el login y
            //si esta registrado cogemos los datos verificamos que este en la lista de usuarios y entramos
                //-> escucha a posteriori
                ...
                when(radioSeleccionado){
                    ...
                    binding.radioRegistrado.id->{
                        //miro los edit y debo buscar en una lista
                        val usuario:Usuario?=usuarios.find { it.correo.
                        equals(binding.editCorreo.text.toString())&&
                                it.pass.equals(binding.editPass.text.toString())
                        }
                        if(usuario==null){
                            Snackbar.make(binding.root,"Error al inicar",Snackbar.LENGTH_SHORT).show()
                        }else{
                            startActivity(intent)
                        }
                    }
